...
| `productByIdProvider` | `FutureProvider.autoDispose.family` | Charge un produit par son id (écran détail) |
...
message + bouton « Réessayer », `data` → l'écran). Le cas d'erreur est
vérifié par un test (un faux repository qui plante → on vérifie que le
message et le bouton s'affichent).

`productByIdProvider` utilise `autoDispose` : quand on quitte l'écran de
détail, le provider se libère tout seul (on n'a plus besoin de garder ce
produit en mémoire).
